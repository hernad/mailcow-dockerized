...
while [[ -z ${DOMAIN_TABLE} ]]; do
  ...
  DOMAIN_TABLE=$(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'domain'" -Bs)
...
while true; do
  ...
  SQL_DOMAINS=$(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain FROM domain WHERE backupmx=0 and active=1" -Bs)
  if [[ ! $? -eq 0 ]]; then
