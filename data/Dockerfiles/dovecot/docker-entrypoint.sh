...
chmod 644 /etc/dovecot/mail_plugins /etc/dovecot/mail_plugins_imap /etc/dovecot/mail_plugins_lmtp 
#chmod 644 /templates/quarantine.tpl
...
function script_init()
  ...
  ltn12 = require "ltn12"
  env = luasql.postgres())
  con = env:connect("__DBNAME__","__DBUSER__","__DBPASS__","__DBHOST__")
  return 0
end
...
sed -i "s/__DBHOST__/${DBHOST}/g" /etc/dovecot/lua/passwd-verify.lua
...
IMAPSYNC_TABLE=$(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'imapsync'" -Bs)
[[ ! -z ${IMAPSYNC_TABLE} ]] && psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "UPDATE imapsync SET is_running='0'"
...
while [[ ${VERSIONS_OK} != 'OK' ]]; do
  if [[ ! -z $(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = \"${DBNAME}\" AND TABLE_NAME = 'versions'") ]]; then
    VERSIONS_OK=OK
  else
