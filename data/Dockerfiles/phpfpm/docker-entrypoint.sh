...
#TZ_CHECK=$(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT CONVERT_TZ('2019-11-02 23:33:00','Europe/Berlin','UTC') AS time;" -BN 2> /dev/null)
...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  do
    DOMAIN_ARR+=("$line")
  done < <(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain FROM domain" -Bs)
  while read line
  do
    DOMAIN_ARR+=("$line")
  done < <(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT alias_domain FROM alias_domain" -Bs)
