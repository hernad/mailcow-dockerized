...
DBV_NOW=$(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT version FROM versions WHERE application = 'db_schema';" -BN)
...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "DROP VIEW IF EXISTS sogo_view"
  while [[ ${VIEW_OK} != 'OK' ]]; do
    psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} << EOF
...
GROUP BY
   mailbox.username;
EOF
    if [[ ! -z $(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'sogo_view'") ]]; then
      VIEW_OK=OK
    else
...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  while [[ ${STATIC_VIEW_OK} != 'OK' ]]; do
    if [[ ! -z $(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '_sogo_static_view'") ]]; then
      STATIC_VIEW_OK=OK
      echo "Updating _sogo_static_view content..."
      # If changed, also update init_db.inc.php
      psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "REPLACE INTO _sogo_static_view (c_uid, domain, c_name, c_password, c_cn, mail, aliases, ad_aliases, ext_acl, kind, multiple_bookings) SELECT c_uid, domain, c_name, c_password, c_cn, mail, aliases, ad_aliases, ext_acl, kind, multiple_bookings from sogo_view;"
      psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "DELETE FROM _sogo_static_view WHERE c_uid NOT IN (SELECT username FROM mailbox WHERE active = '1')"
    else
...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "DROP TRIGGER IF EXISTS sogo_update_password"
...
DELIMITER ;
EOF
    if [[ ! -z $(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_NAME = 'sogo_update_password'") ]]; then
      TRIGGER_OK=OK
    else
...
done < <(psql -H ${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain, CASE gal WHEN '1' THEN 'YES' ELSE 'NO' END AS gal FROM domain;" -B -N)
